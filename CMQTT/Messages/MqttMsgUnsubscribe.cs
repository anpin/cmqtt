...
namespace CMQTT.Messages
{
    ...
    public class MqttMsgUnsubscribe : MqttMsgBase
    {
        ...
        public static MqttMsgUnsubscribe Parse(byte fixedHeaderFirstByte, byte protocolVersion, IMqttNetworkChannel channel)
        {
            ...
            var received = 0;
            buffer = channel.Receive(remainingLength, out received);
            if (received != remainingLength)
                throw new ArgumentOutOfRangeException(string.Format("remainingLength {0} != received {1}", remainingLength, received));
