...
namespace CMQTT
{
    ...
    public class MqttClient
    {
        ...
        {
#if TRACE
            MqttUtility.Trace.Debug("MqttClient>[{0}] starting threads", this.channel.ClientId);
#endif
            this.isRunning = true;

            // start thread for receiving messages from client
            readThread = Fx.StartThread(this.ReadThread);
...
#if TRACE
            MqttUtility.Trace.Debug("MqttClient>[{0}] threads started client", this.channel.ClientId);
#endif
        }
        ...
        //        {
        //#if TRACE
        //            MqttUtility.Trace.Debug("MqttClient: Close was called: [{0}] connected {1} running {2} channel status {3}", this.channel.ClientId, IsConnected,isRunning, channel.Status);
        //#endif
        //            if (this.IsConnected)
        //			{
        //				Stop();
        ...
        {
#if TRACE
            MqttUtility.Trace.Debug("Client> [{0}] is stopping", this.channel.ClientId);
#endif
            if(wasClosed)
            {
#if TRACE
                MqttUtility.Trace.Debug("Client> [{0}] was already stoped", this.channel.ClientId);
                return;
#endif
            }
            ...
            //}
            //else if(isRunning && this.channel.Status != SocketStatus.SOCKET_STATUS_CONNECTED)
            //{

            //    receiveThread.Abort();
            //    dispatchEventThread.Abort();
            //    processInflightThread.Abort();
            //    keepAliveThread.Abort();
#if TRACE
            MqttUtility.Trace.Debug("Client> [{0}] was stopped", this.channel.ClientId);
#endif
        }
        /// <summary>
        ...
        private void Send(byte[] msgBytes)
        {
            ...
            {
                // send message
                this.channel.Send(msgBytes, (sentBytes) =>
                {
        ...
        /// </summary>
        private object ReadThread(object o)
        {
            int read = 0;
            byte[] fixedHeaderFirstByte = new byte[1];
            byte msgType;
#if TRACE
            MqttUtility.Trace.Debug("MqttClient: starting read thread: [{0}] isRunning [{1}]", this.channel.ClientId, this.isRunning);
#endif
...
#if TRACE
                        MqttUtility.Trace.Debug("MqttClient [{0}] read message of type [{1:X}] ", this.channel.ClientId, msgType);
#endif
                        switch (msgType)
                        {
...
#if TRACE
            MqttUtility.Trace.Debug("MqttClient: exiting read thread: [{0}] ", this.channel.ClientId);
#endif
            return null;
        }
